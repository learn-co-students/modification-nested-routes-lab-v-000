class SongsController < ApplicationController
  def index
    if params[:artist_id]
      @artist = Artist.find_by(id: params[:artist_id])
      if @artist.nil?
        redirect_to artists_path, alert: "Artist not found"
      else
        @songs = @artist.songs
      end
    else
      @songs = Song.all
    end
  end

  def show
    if params[:artist_id]
      @artist = Artist.find_by(id: params[:artist_id])
      @song = @artist.songs.find_by(id: params[:id])
      if @song.nil?
        redirect_to artist_songs_path(@artist), alert: "Song not found"
      end
    else
      @song = Song.find(params[:id])
    end
  end

  def new
    if params[:artist_id]
      if !Artist.find_by(id: params[:artist_id]).nil?
        @song = Song.new(artist: Artist.find_by(id: params[:artist_id]))
        # binding.pry
      else
        redirect_to artists_path
      end
    else
      @song = Song.new
    end
  end

  def create
    @song = Song.new(song_params)

    if @song.save
      redirect_to @song
    else
      render :new
    end
  end

  def edit
    # if artist_id exists?
    if params[:artist_id]
      # if artist valid
      if !Artist.find_by(id: params[:artist_id]).nil?
        # check if the song is valid within the artist
        if Artist.find(params[:artist_id]).songs.include?(params[:id])
          # if song found, then song to edit = Song.find(params[:id])
          @song = Song.find(params[:id])
          # edit song nested under artist_id
        else
        # else redirect to artist_songs_path "Song not found"
          redirect_to artist_songs_path(params[:artist_id]), alert: "Song not found."
        end
      # elsif artist not valid, redirect to artists_path, "Artist not found"
      else
        redirect_to artists_path, alert: "Artist not found."
      end
    # elsif params[:artist_id] does not exist(songs/1/edit directly entered)
    else
      @song = Song.find(params[:id])
    end
  end

  def update
    @song = Song.find(params[:id])

    @song.update(song_params)

    if @song.save
      redirect_to @song
    else
      render :edit
    end
  end

  def destroy
    @song = Song.find(params[:id])
    @song.destroy
    flash[:notice] = "Song deleted."
    redirect_to songs_path
  end

  private

  def song_params
    params.require(:song).permit(:title, :artist_name, :artist_id)
  end
end
